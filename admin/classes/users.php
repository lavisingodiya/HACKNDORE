...
class users extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('users', 'users', 'x_id', 'id', '`id`', '`id`', 3, 255, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Fullname
		$this->Fullname = new DbField('users', 'users', 'x_Fullname', 'Fullname', '`Fullname`', '`Fullname`', 200, 100, -1, FALSE, '`Fullname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Fullname->Sortable = TRUE; // Allow sort
		$this->fields['Fullname'] = &$this->Fullname;

		// Username
		$this->Username = new DbField('users', 'users', 'x_Username', 'Username', '`Username`', '`Username`', 200, 100, -1, FALSE, '`Username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Passsword
		$this->Passsword = new DbField('users', 'users', 'x_Passsword', 'Passsword', '`Passsword`', '`Passsword`', 200, 255, -1, FALSE, '`Passsword`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'PASSWORD');
		$this->Passsword->Required = TRUE; // Required field
		$this->Passsword->Sortable = TRUE; // Allow sort
		$this->fields['Passsword'] = &$this->Passsword;

		// userLevelId
		$this->userLevelId = new DbField('users', 'users', 'x_userLevelId', 'userLevelId', '`userLevelId`', '`userLevelId`', 3, 255, -1, FALSE, '`userLevelId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->userLevelId->Lookup = new Lookup('userLevelId', 'userlevels', FALSE, 'userlevelid', ["userlevelname","","",""], [], [], [], [], [], [], '', '');
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->Passsword->DbValue = $row['Passsword'];
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->Passsword->setDbValue($rs->fields('Passsword'));
		...
		if ($Security->canAdmin()) { // System admin
			...
			if ($curVal != "") {
				...
				if ($this->userLevelId->ViewValue === NULL) { // Lookup from database
					...
					$sqlWrk = $this->userLevelId->Lookup->getSql(FALSE, $filterWrk, '', $this);
	...
	public function renderEditRow()
	{
		...
		// Passsword
		$this->Passsword->EditAttrs["class"] = "form-control ew-password-strength";
		$this->Passsword->EditCustomAttributes = "";
		$this->Passsword->EditValue = $this->Passsword->CurrentValue;
		$this->Passsword->PlaceHolder = RemoveHtml($this->Passsword->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->Passsword);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->Passsword);
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
					...
					$message .= '<h3 style="color:;">Hi '.$rsnew['Fullname'].'</h3>';
					$message .= '<p style="font-size:18px;">Your Rates System Account has been created with the following login details;<br/>
					Login ID: <b>'.$rsnew['Username'].'</b> <br/>Password: <b>1qaz!QAZ<p>Please change the password after you login.';
					$message .= '</body></html>';
	...
	function Row_Deleted(&$rs) {
		...
		//echo "Row Deleted";
					...
					// Compose a simple HTML email message
					$message = '<html><body style="">';
					$message .= '<h3 style="color:;">Hi '.$rs['Fullname'].'</h3>';
					$message .= '<p style="font-size:18px;">Your Account has been Deleted, you will nolonger be able to use the Rates System.<br/>';
					$message .= '</body></html>';
